...
pub fn main() !u8 {
    ...
    const cmd_args = blk: {
        ...
        if (all_args.len <= 1) {
            std.log.err("Usage: cc-header-analyzer OUT_DIR -- CC_COMMAND...", .{});
            return 0x7f;
        }
    ...
    const cc_args = blk: {
        for (cmd_args, 0..) |arg, arg_index| {
            ...
            if (!std.mem.startsWith(u8, arg, "-")) {
                if (opt.out_dir) |_| {
                    std.log.err("cc-header-analyzer error: too many non-option cmdline arguments", .{});
                    return 0x7f;
                }
                opt.out_dir = arg;
            } else {

                std.log.err("cc-header-analyzer error: unknown cmdline option '{s}'", .{arg});
                return 0x7f;
            }
        }
        std.log.err("cc-header-analyzer error: missing '--' to delineate CC command", .{});
        return 0x7f;
    };

    const out_dir = opt.out_dir orelse {
        std.log.err("cc-header-analyzer error: missing OUT_DIR command-line argument", .{});
        return 0x7f;
    };
